...
parser = argparse.ArgumentParser(prog = 'PlotTecan', 
                                 description = 'Processes the kinetic measurements produced by multiwell microplate readers of Tecan® (Infinite 200 Pro series) and converts them into tidy tables and graphs. This version focuses only on kinetic spectrophotometric and luminescence measurements',
                                 )
...
parser.add_argument('-o', '--outputname', help = 'Name you want to add to all your output files (e.g. GrowthExperiment -> GrowthExperiment.xlsx, GrowthExperiment_plot.png, etc.)', default = 'result')
parser.add_argument('-m1', '--measurement1', help = 'Name of your first type of measurements (e.g., growth, luminescence)', required = True, default = 'OD')
parser.add_argument('-m2', '--measurement2', help = 'Name of your second type of measurements (e.g., growth, luminescence), if applicable', required = False)
...
check_if_plot = input('\n Do you want to plot the data? [yes/no] ')
check_if_plot = check_if_plot.rstrip()

if check_if_plot.lower() != 'yes':
    sys.exit('\n As you wish! No plots to draw. Exiting the application.')
else: 
    print('\n Warning: All data will be drawn as separate subplots and saved in .pdf format. \n For more plot drawing options use PlotTecan tool.')
...
tdplot.separate_lineplots_w_stdev_as_shadows(replicates, ncols, nrows, fig_size, 
                                      ...
                                      ylabel = 'OD[AU]', same_yscale=True)

plt.savefig(outputname + '_' + keyword1 + "_sep_plot" + ".pdf", format = "pdf")
plt.show()
